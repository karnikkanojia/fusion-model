...
def train(model, dataset, cfg, labels_shape):
    print("Our config:")
    pprint.pprint(cfg)
    ...
    optim = torch.optim.Adam(
        model.parameters(), lr=cfg.lr, weight_decay=1e-5, amsgrad=True
    )
    print(optim)
    ...
    metrics = []
    weights_files = glob(
        join(cfg.output_dir, f"{dataset_name}-e*.pt")
    ...
    if len(weights_files):
        ...
        epochs = np.array(
            [
                int(
                    w[
                        len(join(cfg.output_dir, f"{dataset_name}-e")) : -len(".pt")
                    ].split("-")[0]
                )
        ...
        weights_file = [
            weights_files[i] for i in np.argwhere(epochs == np.amax(epochs)).flatten()
        ][0]
...
def train_epoch(
    cfg, epoch, model, device, train_loader, optimizer, criterion, limit=None, labels_shape=18
):
    model.feature_extractor.densenet.to(device)
    model.feature_extractor.resnet.to(device)
    model.train()
    ...
    for batch_idx, samples in enumerate(t):
        ...
        targets = samples["lab"].float().to(device)
    ...
    with torch.no_grad():
        ...
        for task in range(len(task_targets)):
            if len(np.unique(task_targets[task])) > 1:
                task_auc = sklearn.metrics.roc_auc_score(
                    task_targets[task], task_outputs[task]
                )
                # print(task, task_auc)
